...
bool win_filesystem::CreateFilePathList(const wchar_t* pwzFolderPath, const wchar_t* pwzFileSpec, std::vector<std::wstring>& paths)
{
	...
	{
		if (wcschr(pwzFileSpec, L'*') == nullptr)
		{
			wstrPath += L'*';
		}
		wstrPath += pwzFileSpec;
	}
	else
	{
		wstrPath += L'*';
...
void win_filesystem::GetFilePathListAndIndex(const std::wstring& wstrPath, const wchar_t* pwzFileSpec, std::vector<std::wstring>& folders, size_t* nIndex)
{
	...
	win_filesystem::CreateFilePathList(wstrParent.c_str(), pwzFileSpec, folders);
